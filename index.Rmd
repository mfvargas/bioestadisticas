...
```{r natacion_real_peso, message=FALSE}
natacion_real_peso <-
  bioestadisticas %>%
...
### Fecha y natación (distancia real)

```{r fecha-natacion_real, message=FALSE}
fecha_natacion_real <-
  bioestadisticas %>%
  filter(!is.na(`nat-distancia-real`)) %>%
  ggplot(aes(x = fecha, y = `nat-distancia-real`)) +
  geom_line(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  theme_minimal()

ggplotly(fecha_natacion_real)
```
